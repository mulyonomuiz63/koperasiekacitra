...
<div class="card card-flush">
    ...
    <div class="card-header align-items-center gap-2 gap-md-5">
        <div class="card-title">
            <div class="d-flex align-items-center position-relative my-1">
                ...
                <input type="text" data-kt-ecommerce-order-filter="search" class="form-control form-control-solid w-250px ps-12" placeholder="Search news">
            </div>
        </div>
        <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
            <?php if (can($menuId, 'create')): ?>
                <a href="<?= base_url('news/create') ?>" class="btn btn-primary btn-sm">+ Tambah news</a>
            <?php endif; ?>
        </div>
    </div>
    <!--end::Card header-->
    ...
    <div class="card-body pt-0">
        <div class="table-responsive">
            <table class="table table-row-dashed table-row-gray-300 align-middle fs-6 gy-4" id="kt_news_table">
                <thead>
...
$(document).ready(function () {
    ...
    let table = $('#kt_news_table').DataTable({
        processing: true,
        serverSide: true,
        ordering: false,
        ajax: {
            url: "<?= base_url('news/datatable') ?>",
            type: "POST",
        },
        columns: [
            { data: 'title' },
            ...
            { data: 'filename' }, // Sudah berisi <img> dari controller
            {
                ...
                render: function (data, type, row) {
                    let btnDelete = `
                        ...
                        data-url="<?= base_url('news/delete') ?>/${row.id}"
                    ...
                    let btnEdit = `
                        <a href="<?= base_url('news/edit') ?>/${row.id}"
